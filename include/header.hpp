...
public:
    ...
    void print(const std::string &number_Broker) {
            std::cout << "Broker:" << _nameBroker << ' '
                      << "Account:" <<  _account[number_Broker].numberBroker << ' '
                      << "Files:" <<  _account[number_Broker].filesBroker << ' '
                      << "Last date:" << _account[number_Broker].lastDateBroker << std::endl;
    }
};
            ...
            if (exists(x.path())) {
                if (is_directory(x.path())) {
                    ...
                    if (trueFile(x.path())) {
                        ...
                        auto broker_number = x.path().stem().string().substr(8, 8);
                        auto date =
